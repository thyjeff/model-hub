...
Model Hub Proxy is a Node.js proxy server that exposes an Anthropic-compatible API backed by Google Cloud Code service. It enables using Claude models (`claude-sonnet-4-5-thinking`, `claude-opus-4-6-thinking`) and Gemini models (`gemini-3-flash`, `gemini-3.1-pro-low`, `gemini-3.1-pro-high`) with Claude Code CLI.

The proxy translates requests from Anthropic Messages API format â†’ Google Generative AI format â†’ Google Cloud Code API, then converts responses back to Anthropic format with full thinking/streaming support.
...
**Request Flow:**
```
Claude Code CLI â†’ Express Server (server.js) â†’ CloudCode Client â†’ Google Cloud Code API
```
...
â”œâ”€â”€ constants.js                # Configuration values
â”œâ”€â”€ errors.js                   # Custom error classes
â”œâ”€â”€ fallback-config.js          # Model fallback mappings and helpers
â”‚
â”œâ”€â”€ cloudcode/                  # Cloud Code API client
â”‚   â”œâ”€â”€ index.js                # Public API exports
â”‚   â”œâ”€â”€ session-manager.js      # Session ID derivation for caching
â”‚   â”œâ”€â”€ rate-limit-parser.js    # Parse reset times from headers/errors
â”‚   â”œâ”€â”€ request-builder.js      # Build API request payloads
â”‚   â”œâ”€â”€ sse-parser.js           # Parse SSE for non-streaming
â”‚   â”œâ”€â”€ sse-streamer.js         # Stream SSE events in real-time
â”‚   â”œâ”€â”€ message-handler.js      # Non-streaming message handling
â”‚   â”œâ”€â”€ streaming-handler.js    # Streaming message handling
â”‚   â””â”€â”€ model-api.js            # Model listing and quota APIs
â”‚
â”œâ”€â”€ account-manager/            # Multi-account pool management
â”‚   â”œâ”€â”€ index.js                # AccountManager class facade
â”‚   â”œâ”€â”€ storage.js              # Config file I/O and persistence
â”‚   â”œâ”€â”€ rate-limits.js          # Rate limit tracking and state
â”‚   â”œâ”€â”€ credentials.js          # OAuth token and project handling
â”‚   â””â”€â”€ strategies/             # Account selection strategies
â”‚       â”œâ”€â”€ index.js            # Strategy factory (createStrategy)
â”‚       â”œâ”€â”€ base-strategy.js    # Abstract base class
â”‚       â”œâ”€â”€ sticky-strategy.js  # Cache-optimized sticky selection
â”‚       â”œâ”€â”€ round-robin-strategy.js  # Load-balanced rotation
â”‚       â”œâ”€â”€ hybrid-strategy.js  # Smart multi-signal distribution
â”‚       â””â”€â”€ trackers/           # State trackers for hybrid strategy
â”‚           â”œâ”€â”€ index.js        # Re-exports trackers
â”‚           â”œâ”€â”€ health-tracker.js    # Account health scores
â”‚           â”œâ”€â”€ token-bucket-tracker.js  # Client-side rate limiting
â”‚           â””â”€â”€ quota-tracker.js     # Quota-aware account selection
â”‚
â”œâ”€â”€ auth/                       # Authentication
â”‚   â”œâ”€â”€ oauth.js                # Google OAuth with PKCE
â”‚   â”œâ”€â”€ token-extractor.js      # Legacy token extraction from DB
â”‚   â””â”€â”€ database.js             # SQLite database access
â”‚
â”œâ”€â”€ webui/                      # Web Management Interface
â”‚   â””â”€â”€ index.js                # Express router and API endpoints
â”‚
â”œâ”€â”€ modules/                    # Feature modules
â”‚   â””â”€â”€ usage-stats.js          # Request tracking and history persistence
â”‚
â”œâ”€â”€ cli/                        # CLI tools
â”‚   â””â”€â”€ accounts.js             # Account management CLI
â”‚
â”œâ”€â”€ format/                     # Format conversion (Anthropic â†” Google)
â”‚   â”œâ”€â”€ index.js                # Re-exports all converters
â”‚   â”œâ”€â”€ request-converter.js    # Anthropic â†’ Google conversion
â”‚   â”œâ”€â”€ response-converter.js   # Google â†’ Anthropic conversion
â”‚   â”œâ”€â”€ content-converter.js    # Message content conversion
â”‚   â”œâ”€â”€ schema-sanitizer.js     # JSON Schema cleaning for Gemini
â”‚   â”œâ”€â”€ thinking-utils.js       # Thinking block validation/recovery
â”‚   â””â”€â”€ signature-cache.js      # Signature cache (tool_use + thinking signatures)
â”‚
â””â”€â”€ utils/                      # Utilities
    â”œâ”€â”€ claude-config.js        # Claude CLI settings file I/O (supports CLAUDE_CONFIG_PATH env var)
    â”œâ”€â”€ helpers.js              # formatDuration, sleep, isNetworkError
    â”œâ”€â”€ logger.js               # Structured logging
    â””â”€â”€ native-module-helper.js # Auto-rebuild for native modules
```
...
â”œâ”€â”€ css/
â”‚   â”œâ”€â”€ style.css               # Compiled Tailwind CSS (generated, do not edit)
â”‚   â””â”€â”€ src/
â”‚       â””â”€â”€ input.css           # Tailwind source with @apply directives
â”œâ”€â”€ js/
â”‚   â”œâ”€â”€ app.js                  # Main application logic (Alpine.js)
â”‚   â”œâ”€â”€ config/                 # Application configuration
â”‚   â”‚   â””â”€â”€ constants.js        # Centralized UI constants and limits
â”‚   â”œâ”€â”€ store.js                # Global state management
â”‚   â”œâ”€â”€ data-store.js           # Shared data store (accounts, models, quotas, placeholder data)
â”‚   â”œâ”€â”€ settings-store.js       # Settings management store (incl. dev mode sub-toggles)
â”‚   â”œâ”€â”€ components/             # UI Components
â”‚   â”‚   â”œâ”€â”€ dashboard.js        # Main dashboard orchestrator
â”‚   â”‚   â”œâ”€â”€ account-manager.js  # Account list, OAuth, & threshold settings
â”‚   â”‚   â”œâ”€â”€ models.js           # Model list with draggable quota threshold markers
â”‚   â”‚   â”œâ”€â”€ logs-viewer.js      # Live log streaming
â”‚   â”‚   â”œâ”€â”€ claude-config.js    # CLI settings editor
â”‚   â”‚   â”œâ”€â”€ server-config.js    # Server settings UI
â”‚   â”‚   â””â”€â”€ dashboard/          # Dashboard sub-modules
â”‚   â”‚       â”œâ”€â”€ stats.js        # Account statistics calculation
â”‚   â”‚       â”œâ”€â”€ charts.js       # Chart.js visualizations
â”‚   â”‚       â””â”€â”€ filters.js      # Chart filter state management
â”‚   â””â”€â”€ utils/                  # Frontend utilities
â”‚       â”œâ”€â”€ error-handler.js    # Centralized error handling with ErrorHandler.withLoading
â”‚       â”œâ”€â”€ account-actions.js  # Account operations service layer
â”‚       â”œâ”€â”€ redact.js           # Screenshot mode email redaction utility
â”‚       â”œâ”€â”€ validators.js       # Input validation
â”‚       â””â”€â”€ model-config.js     # Model configuration helpers
...
- Account state persisted to `~/.config/modelhub-proxy/accounts.json`
...
3. **Hybrid Strategy** (default, smart distribution):
   ...
   - Scoring formula: `score = (Health Ã— 2) + ((Tokens / MaxTokens Ã— 100) Ã— 5) + (Quota Ã— 1) + (LRU Ã— 0.1)`
...
- Global threshold configurable via WebUI Settings â†’ Quota Protection
...
- Thinking models fall back to thinking models (e.g., `claude-sonnet-4-5-thinking` â†’ `gemini-3-flash`)
...
- `hasGeminiHistory()` detects Geminiâ†’Claude cross-model scenarios
...
- Pattern inspired by upstream manager\x27s `clean_cache_control_from_messages()`
...
- **State Management**:
  ...
  - Layered architecture: Service Layer (`account-actions.js`) â†’ Component Layer â†’ UI
- **Features**:
...
- **Smart Refresh**: Client-side polling with Â±20% jitter and tab visibility detection (3x slower when hidden)
- **i18n Support**: English, Chinese (ä¸­æ–‡), Indonesian (Bahasa), Portuguese (PT-BR), Turkish (TÃ¼rkÃ§e)
...
// âŒ Bad: Direct API call in component
