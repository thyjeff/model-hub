...
- Model Hub Proxy running on port 8080
...
```json
{
  ...
  "agents": {
    "defaults": {
      "model": {
        "primary": "modelhub-proxy/gemini-3-flash",
        "fallbacks": ["modelhub-proxy/gemini-3.1-pro-high"]
      },
      "models": {
        "modelhub-proxy/gemini-3-flash": {}
      }
    }
  }
}
...
You should see models prefixed with `modelhub-proxy/` in the list.
...
```bash
openclaw models set modelhub-proxy/claude-opus-4-6-thinking
```
...
HOST=127.0.0.1 model-hub-proxy start
```
