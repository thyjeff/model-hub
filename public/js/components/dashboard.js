...
window.Components.dashboard = () => ({
    ...
    ollamaUsage: { totals: { requests: 0, inputTokens: 0, outputTokens: 0, totalTokens: 0 }, topModels: [], upstreams: [], recentSessions: [] },
    historyData: {},
    modelTree: {},
    families: [],
    ...
    init() {
        ...
        this.$watch('$store.data.ollamaUsage', (nextUsage) => {
            if (this.$store.global.activeTab === 'dashboard' && nextUsage) {
                this.ollamaUsage = nextUsage;
            }
        });
        ...
        if (this.$store.global.activeTab === 'dashboard') {
            this.$nextTick(() => {
                ...
                const usage = Alpine.store('data').ollamaUsage;
                if (usage) this.ollamaUsage = usage;
            });
        }
    },
