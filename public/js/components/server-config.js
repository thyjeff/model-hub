...
window.Components.serverConfig = () => ({
    ...
    async fetchServerConfig() {
        ...
        try {
            ...
            if (!Array.isArray(this.serverConfig.ollamaBaseUrls)) this.serverConfig.ollamaBaseUrls = [];
            if (typeof this.serverConfig.ollamaBaseUrl !== 'string') this.serverConfig.ollamaBaseUrl = '';
            if (typeof this.serverConfig.ollamaCloudBaseUrl !== 'string') this.serverConfig.ollamaCloudBaseUrl = '';
            if (!Array.isArray(this.serverConfig.ollamaApiKeys)) this.serverConfig.ollamaApiKeys = [];
            if (typeof this.serverConfig.ollamaFailoverCooldownMs !== 'number') this.serverConfig.ollamaFailoverCooldownMs = 30000;
        } catch (e) {
    ...
    async saveOllamaBaseUrl(value) {
        const store = Alpine.store('global');
        const nextValue = String(value || '').trim();
        const prev = this.serverConfig.ollamaBaseUrl || '';
        this.serverConfig.ollamaBaseUrl = nextValue;
        try {
            const { response, newPassword } = await window.utils.request('/api/config', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ollamaBaseUrl: nextValue })
            }, store.webuiPassword);
            if (newPassword) store.webuiPassword = newPassword;
            const data = await response.json();
            if (data.status !== 'ok') throw new Error(data.error || 'Failed to update Ollama base URL');
            store.showToast('Ollama base URL saved', 'success');
            await this.fetchServerConfig();
        } catch (e) {
            this.serverConfig.ollamaBaseUrl = prev;
            store.showToast('Failed to save Ollama base URL: ' + e.message, 'error');
        }
    },

    async saveOllamaBaseUrls(rawInput) {
        const store = Alpine.store('global');
        const list = String(rawInput || '')
            .split(/[\n,]/)
            .map(v => v.trim())
            .filter(Boolean);
        const unique = Array.from(new Set(list)).slice(0, 100);
        const prev = Array.isArray(this.serverConfig.ollamaBaseUrls) ? [...this.serverConfig.ollamaBaseUrls] : [];
        this.serverConfig.ollamaBaseUrls = unique;
        try {
            const { response, newPassword } = await window.utils.request('/api/config', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ollamaBaseUrls: unique })
            }, store.webuiPassword);
            if (newPassword) store.webuiPassword = newPassword;
            const data = await response.json();
            if (data.status !== 'ok') throw new Error(data.error || 'Failed to update Ollama upstream pool');
            store.showToast('Ollama upstream pool saved', 'success');
            await this.fetchServerConfig();
        } catch (e) {
            this.serverConfig.ollamaBaseUrls = prev;
            store.showToast('Failed to save Ollama upstream pool: ' + e.message, 'error');
        }
    },

    toggleOllamaFailoverCooldownMs(value) {
        this.saveConfigField('ollamaFailoverCooldownMs', value, 'Ollama Failover Cooldown',
            (v) => window.Validators.validateRange(v, 1000, 300000, 'Ollama Failover Cooldown'));
    },
    ...
    async executeSaveServerPreset(name) {
        ...
        try {
            ...
            const relevantKeys = [
                ...
                'ollamaBaseUrl', 'ollamaBaseUrls', 'ollamaCloudBaseUrl', 'ollamaApiKeys', 'ollamaFailoverCooldownMs',
                'accountSelection'
            ];
            const presetConfig = {};
    ...
    flattenConfigToDraft(cfg) {
        ...
        return {
            ...
            ollamaBaseUrls: Array.isArray(cfg.ollamaBaseUrls) ? cfg.ollamaBaseUrls.join(', ') : '',
            ollamaCloudBaseUrl: cfg.ollamaCloudBaseUrl || '',
            ollamaApiKeys: Array.isArray(cfg.ollamaApiKeys) ? cfg.ollamaApiKeys.join(', ') : '',
            ollamaFailoverCooldownMs: cfg.ollamaFailoverCooldownMs,
    ...
    buildConfigFromDraft(draft) {
        const cfg = {
            ...
            ollamaCloudBaseUrl: draft.ollamaCloudBaseUrl || '',
            ollamaFailoverCooldownMs: draft.ollamaFailoverCooldownMs,
        };

        if (typeof draft.ollamaBaseUrls === 'string' && draft.ollamaBaseUrls.trim()) {
            cfg.ollamaBaseUrls = Array.from(
                new Set(
                    draft.ollamaBaseUrls
                        .split(/[,\n]/)
                        .map(s => s.trim())
                        .filter(Boolean)
                )
        ...
        if (typeof draft.ollamaApiKeys === 'string' && draft.ollamaApiKeys.trim()) {
            cfg.ollamaApiKeys = Array.from(
                new Set(
                    draft.ollamaApiKeys
                        .split(/[,\n]/)
                        .map(s => s.trim())
