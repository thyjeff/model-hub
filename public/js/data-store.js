...
document.addEventListener('alpine:init', () => {
    Alpine.store('data', {
        ...
        ollamaUsage: { totals: {}, topModels: [], upstreams: [], recentSessions: [] },
        ...
        init() {
            ...
            try {
                const saved = JSON.parse(localStorage.getItem('modelhub_settings') || localStorage.getItem('antigravity_settings') || '{}');
            ...
            try {
                ...
                if (cached) {
                    ...
                    if (data.accounts && data.models) {
                        ...
                        this.ollamaUsage = data.ollamaUsage || this.ollamaUsage;
        ...
        async fetchData() {
            ...
            try {
                ...
                // Merge explicit model catalog (includes Ollama) for UI selectors
                try {
                    const { response: modelsResp, newPassword: pw2 } = await window.utils.request('/api/models/all', {}, password);
                    if (pw2) Alpine.store('global').webuiPassword = pw2;
                    if (modelsResp.ok) {
                        const modelsPayload = await modelsResp.json();
                        const merged = new Set([...(this.models || []), ...((modelsPayload.models || []))]);
                        this.models = Array.from(merged).sort();
                    }
                } catch (_) {
                    // Non-blocking: keep model list from /account-limits when catalog fetch fails
                }
                ...
                if (data.ollamaUsage) {
                    this.ollamaUsage = data.ollamaUsage;
                }

                // Always refresh Ollama usage directly as well (independent of cloud quota path)
                try {
                    const { response: ollamaUsageResp, newPassword: pw3 } = await window.utils.request('/api/ollama/usage', {}, password);
                    if (pw3) Alpine.store('global').webuiPassword = pw3;
                    if (ollamaUsageResp.ok) {
                        const payload = await ollamaUsageResp.json();
                        if (payload?.usage) this.ollamaUsage = payload.usage;
                    }
                } catch (_) {
                    // Non-blocking
                }
                ...
                // Fallback: still try loading Ollama usage so dashboard has local stats
                try {
                    const password = Alpine.store('global').webuiPassword;
                    const { response: ollamaUsageResp, newPassword: pwFallback } = await window.utils.request('/api/ollama/usage', {}, password);
                    if (pwFallback) Alpine.store('global').webuiPassword = pwFallback;
                    if (ollamaUsageResp.ok) {
                        const payload = await ollamaUsageResp.json();
                        if (payload?.usage) this.ollamaUsage = payload.usage;
                    }
        ...
        getModelFamily(modelId) {
            ...
            if (lower.startsWith('ollama/') || lower.includes('ollama')) return 'ollama';
            if (lower.startsWith('openai/') || lower.includes('gpt-')) return 'openai';
            if (lower.startsWith('kimi/') || lower.includes('moonshot') || lower.includes('kimi-')) return 'kimi';
