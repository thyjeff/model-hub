/**
 * Constants for Model Hub Cloud Code API integration
 * Based on upstream OAuth/client compatibility research
 */
...
/**
 * Get the desktop-app database path based on the current platform.
 * - macOS: app support folder for the desktop app
 * - Windows: roaming app-data folder for the desktop app
 * - Linux/other: config folder for the desktop app
 * @returns {string} Full path to the desktop-app state database
 */
function getModelHubDbPath() {
    const home = homedir();
    switch (platform()) {
        case 'darwin':
...
// Reference: desktop-app binary analysis - google.internal.cloud.code.v1internal.ClientMetadata.IdeType
...
// Platform enum (as specified in desktop-app binary)
...
// Plugin type enum (as specified in desktop-app binary)
...
export const CLIENT_METADATA = {
    ideType: IDE_TYPE.MODEL_HUB,   // 6 - identifies as desktop-app client (protocol-required value)
    platform: getPlatformEnum(),      // Runtime platform detection
    pluginType: PLUGIN_TYPE.GEMINI    // 2
};

// Cloud Code API endpoints (in fallback order)
const MODEL_HUB_ENDPOINT_DAILY = 'https://daily-cloudcode-pa.googleapis.com';
const MODEL_HUB_ENDPOINT_PROD = 'https://cloudcode-pa.googleapis.com';

// Endpoint fallback order (daily ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ prod)
export const MODEL_HUB_ENDPOINT_FALLBACKS = [
    MODEL_HUB_ENDPOINT_DAILY,
    MODEL_HUB_ENDPOINT_PROD
];

// Required headers for Cloud Code API requests
// Headers for general Cloud Code API requests
...
export const ONBOARD_USER_ENDPOINTS = MODEL_HUB_ENDPOINT_FALLBACKS;

// Headers for loadCodeAssist API
// Matches the minimal headers seen in the binary's u() method
export const LOAD_CODE_ASSIST_HEADERS = MODEL_HUB_HEADERS;
...
export const MODEL_HUB_AUTH_PORT = 9092;
export const DEFAULT_PORT = config?.port || 8080;

// Multi-account configuration
export const ACCOUNT_CONFIG_PATH = config?.accountConfigPath || join(
    homedir(),
    '.config/modelhub-proxy/accounts.json'
);

// Usage history persistence path
export const USAGE_HISTORY_PATH = join(
    homedir(),
    '.config/modelhub-proxy/usage-history.json'
);

// Desktop app database path (for legacy single-account token extraction)
// Uses platform-specific path detection
export const MODEL_HUB_DB_PATH = getModelHubDbPath();
...
// 0 means "wait indefinitely" (never fail fast on long cooldown windows).
...
// Minimum backoff floor to prevent "Available in 0s" loops (matches upstream auth behavior)
...
export const CAPACITY_JITTER_MAX_MS = 10000; // Ãƒâ€šÃ‚Â±5s jitter range
...
// Google OAuth configuration (from upstream auth flow)
...
const DEFAULT_GOOGLE_OAUTH_CLIENT_ID = process.env.GOOGLE_CLIENT_ID || '';
const GOOGLE_OAUTH_CLIENT_ID = DEFAULT_GOOGLE_OAUTH_CLIENT_ID;
...
// Minimal system instruction (from CLIProxyAPI)
...
export const MODEL_HUB_SYSTEM_INSTRUCTION = `You are Model Hub Assistant, a powerful agentic AI coding assistant.You are pair programming with a USER to solve their coding task. The task may require creating a new codebase, modifying or debugging an existing codebase, or simply answering a question.**Absolute paths only****Proactiveness**`;
