...
const DEFAULT_CONFIG = {
    ...
    // Ollama upstream failover pool. If multiple URLs are provided, requests
    // will switch to another upstream on capacity/quota/network failures.
    ollamaBaseUrl: '',
    ollamaBaseUrls: [],
    ollamaCloudBaseUrl: '',
    ollamaApiKeys: [],
    ...
    kimiBaseUrl: '',
    kimiApiKeys: [],
    ollamaFailoverCooldownMs: 30000,
    modelMapping: {},
...
const CONFIG_DIR = path.join(HOME_DIR, '.config', 'modelhub-proxy');
...
function loadConfig() {
    try {
        ...
        if (process.env.OLLAMA_CLOUD_BASE_URL) config.ollamaCloudBaseUrl = process.env.OLLAMA_CLOUD_BASE_URL;
        if (process.env.OLLAMA_API_KEYS) {
            config.ollamaApiKeys = process.env.OLLAMA_API_KEYS.split(',').map(v => String(v || '').trim()).filter(Boolean);
        }
        if (process.env.OPENAI_BASE_URL) config.openaiBaseUrl = process.env.OPENAI_BASE_URL;
        if (process.env.OPENAI_API_KEYS) {
            config.openaiApiKeys = process.env.OPENAI_API_KEYS.split(',').map(v => String(v || '').trim()).filter(Boolean);
        }
        if (process.env.KIMI_BASE_URL) config.kimiBaseUrl = process.env.KIMI_BASE_URL;
        if (process.env.KIMI_API_KEYS) {
            config.kimiApiKeys = process.env.KIMI_API_KEYS.split(',').map(v => String(v || '').trim()).filter(Boolean);
        }
