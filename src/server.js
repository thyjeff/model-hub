...
import { isOllamaModel, listOllamaModels, listOpenAIModels, listKimiModels, sendOllamaMessage, sendOllamaMessageStream, getOllamaUsageStats } from './ollama/index.js';
import { openAiToAnthropicRequest, anthropicToOpenAiChatResponse, createChatStreamState, anthropicEventToOpenAiChatChunk, openAiResponsesInputToChatMessages, chatResponseToResponsesResponse, openAiError } from './openai-compat.js';
...
function parseError(error) {
    ...
    if (error.message.includes('Provider API key missing') || error.message.includes('No upstream configured')) {
        errorType = 'invalid_request_error';
        statusCode = 400;
        errorMessage = error.message;
    } else if (
        error.message.includes('authorization was missing') ||
        error.message.includes('invalid_api_key') ||
        error.message.includes('Incorrect API key provided')
    ) {
        errorType = 'authentication_error';
        statusCode = 401;
        errorMessage = 'Provider authentication failed. Check OpenAI/Kimi API key pools and base URLs in Settings.';
...
function extractSessionId(req, body = {}) {
    ...
    const value = String(fromBody || fromHeaders || 'unknown').trim();
    return value.slice(0, 120);
}
function inferProviderModel(rawModelId) {
    const raw = String(rawModelId || '').trim();
    if (!raw) return raw;

    const lower = raw.toLowerCase();
    const hasKnownPrefix = lower.startsWith('ollama/') || lower.startsWith('openai/') || lower.startsWith('kimi/');
    if (hasKnownPrefix) return raw;
    const unprefixed = raw;
    const unprefixedLower = unprefixed.toLowerCase();

    const isOpenAIStyle =
        unprefixedLower.startsWith('gpt-') ||
        unprefixedLower.startsWith('o1') ||
        unprefixedLower.startsWith('o3') ||
        unprefixedLower.startsWith('o4');
    const isKimiStyle =
        unprefixedLower.includes('kimi') ||
        unprefixedLower.includes('moonshot');

    if (isOpenAIStyle) return `openai/${unprefixed}`;
    if (isKimiStyle) return `kimi/${unprefixed}`;
    return `ollama/${unprefixed}`;
}
...
app.get('/account-limits', async (req, res) => {
    try {
        ...
        // Include OpenAI-compatible provider models in UI model lists even without cloud quota rows
        for (const source of [
            ...
            try {
                ...
                for (const model of payload?.data || []) {
                    if (model?.id) allModelIds.add(model.id);
                }
            } catch (error) {
                logger.warn(`[API] Unable to include ${source.name} models in account-limits: ${error.message}`);
            }
        }
        ...
        responseData.ollamaUsage = getOllamaUsageStats();
...
/**
 * Ollama usage stats endpoint
 */
app.get('/api/ollama/usage', (req, res) => {
    try {
        res.json({
            status: 'ok',
            usage: getOllamaUsageStats()
        });
    } catch (error) {
        res.status(500).json({
            status: 'error',
...
app.get('/v1/models', async (req, res) => {
    try {
        ...
        try {
            ...
            if (account) {
                const token = await accountManager.getTokenForAccount(account);
                const cloud = await listModels(token);
                cloudModels = Array.isArray(cloud?.data) ? cloud.data : [];
            }
        } catch (cloudError) {
            logger.warn('[API] Cloud model listing unavailable:', cloudError.message);
        }

        try {
            const ollama = await listOllamaModels();
            ollamaModels = Array.isArray(ollama?.data) ? ollama.data : [];
        } catch (ollamaError) {
            logger.warn('[API] Ollama model listing unavailable:', ollamaError.message);
        }
        try {
            const openai = await listOpenAIModels();
            openaiModels = Array.isArray(openai?.data) ? openai.data : [];
        } catch (openaiError) {
            logger.warn('[API] OpenAI model listing unavailable:', openaiError.message);
        }

        try {
            const kimi = await listKimiModels();
            kimiModels = Array.isArray(kimi?.data) ? kimi.data : [];
        } catch (kimiError) {
            logger.warn('[API] Kimi model listing unavailable:', kimiError.message);
        }

        const deduped = new Map();
        for (const model of [...cloudModels, ...ollamaModels, ...openaiModels, ...kimiModels]) {
            if (model?.id) deduped.set(model.id, model);
        }

        // Always include baseline models so clients can select and send requests
        // even when upstream catalog endpoints are temporarily unavailable.
...
/**
 * OpenAI-compatible Chat Completions endpoint (for Codex CLI compatibility)
 */
app.post('/v1/chat/completions', async (req, res) => {
    try {
        const anthropicRequest = openAiToAnthropicRequest(req.body || {});
        anthropicRequest.session_id = extractSessionId(req, req.body || {});

        // Resolve model mapping if configured
        let requestedModel = anthropicRequest.model || 'ollama/deepseek-v3.1:671b-cloud';
        ...
        requestedModel = inferProviderModel(requestedModel);
        anthropicRequest.model = requestedModel;

        const isOllamaRequest = isOllamaModel(requestedModel);
        ...
        if (!isOllamaRequest) {
            ...
            if (validationAccount) {
                ...
                const valid = await isValidModel(requestedModel, token, projectId);
                if (!valid) {
                    return res.status(400).json(openAiError('Invalid model: ' + requestedModel + '. Use /v1/models to see available models.', 'invalid_request_error'));
                }
            }
        ...
        if (!Array.isArray(anthropicRequest.messages) || anthropicRequest.messages.length === 0) {
            return res.status(400).json(openAiError('messages is required and must be a non-empty array', 'invalid_request_error'));
        }

        if (anthropicRequest.stream) {
            const generator = isOllamaRequest
                ? sendOllamaMessageStream(anthropicRequest)
                : sendMessageStream(anthropicRequest, accountManager, FALLBACK_ENABLED);

            const streamState = createChatStreamState(requestedModel);
            ...
            const first = anthropicEventToOpenAiChatChunk({ type: "message_start" }, streamState);
            if (first) {
                res.write("data: " + JSON.stringify(first) + "\n\n");
            }

            for await (const event of generator) {
                const chunk = anthropicEventToOpenAiChatChunk(event, streamState);
                if (chunk) {
        ...
        const anthropicResponse = isOllamaRequest
            ? await sendOllamaMessage(anthropicRequest)
            : await sendMessage(anthropicRequest, accountManager, FALLBACK_ENABLED);

        return res.json(anthropicToOpenAiChatResponse(anthropicResponse));
    } catch (error) {
        logger.error('[API] /v1/chat/completions error:', error);
        const { errorType, statusCode, errorMessage } = parseError(error);
        return res.status(statusCode || 500).json(openAiError(errorMessage || error.message, errorType || 'api_error'));
    }
});

/**
 * OpenAI-compatible Responses endpoint (for Codex CLI compatibility)
 */
app.post('/v1/responses', async (req, res) => {
    try {
        ...
        const chatBody = {
            model: body.model,
            messages: openAiResponsesInputToChatMessages(body.input),
            tools: body.tools,
        ...
        const anthropicRequest = openAiToAnthropicRequest(chatBody);
        anthropicRequest.session_id = extractSessionId(req, body || {});
        let requestedModel = anthropicRequest.model || "ollama/deepseek-v3.1:671b-cloud";
        const modelMapping = config.modelMapping || {};
        if (modelMapping[requestedModel] && modelMapping[requestedModel].mapping) {
        ...
        if (!isOllamaRequest) {
            ...
            if (validationAccount) {
                ...
                const valid = await isValidModel(requestedModel, token, projectId);
                if (!valid) {
                    return res.status(400).json(openAiError('Invalid model: ' + requestedModel, 'invalid_request_error'));
                }
            }
        }
        ...
        if (body.stream) {
            ...
            const responseId = "resp_" + Math.random().toString(36).slice(2, 14);
            ...
            res.write("data: " + JSON.stringify({ type: "response.created", response: { id: responseId, object: "response", status: "in_progress", model: requestedModel } }) + "\n\n");

            for await (const event of generator) {
                if (event?.type === "content_block_delta" && event?.delta?.type === "text_delta") {
                    res.write("data: " + JSON.stringify({ type: "response.output_text.delta", delta: event.delta.text || "" }) + "\n\n");
                }
                if (event?.type === "message_stop") {
                    res.write("data: " + JSON.stringify({ type: "response.completed", response: { id: responseId, object: "response", status: "completed", model: requestedModel } }) + "\n\n");
                }
            }
        ...
        const openAiChat = anthropicToOpenAiChatResponse(anthropicResponse);
        return res.json(chatResponseToResponsesResponse(openAiChat));
    } catch (error) {
        logger.error('[API] /v1/responses error:', error);
        const { errorType, statusCode, errorMessage } = parseError(error);
        return res.status(statusCode || 500).json(openAiError(errorMessage || error.message, errorType || 'api_error'));
    }
});
...
app.post('/v1/messages', async (req, res) => {
    try {
        ...
        const modelId = inferProviderModel(requestedModel);

        const isOllamaRequest = isOllamaModel(modelId);
        ...
        request.session_id = extractSessionId(req, req.body || {});
        ...
        if (stream) {
            ...
            try {
                ...
                const generator = isOllamaRequest ? sendOllamaMessageStream(request) : sendMessageStream(request, accountManager, FALLBACK_ENABLED);
            ...
            const response = isOllamaRequest ? await sendOllamaMessage(request) : await sendMessage(request, accountManager, FALLBACK_ENABLED);
            res.json(response);
        }
