...
import { readCodexProxyConfig, setCodexProxyConfig, setCodexPaidMode, getCodexConfigPath, readCodexPresets, saveCodexPreset, deleteCodexPreset } from '../utils/codex-config.js';
...
import { listOllamaModels, listOpenAIModels, listKimiModels, sendOllamaMessage } from '../ollama/index.js';
...
async function getPreferredClaudeConfig() {
    const globalPath = getClaudeConfigPath();
    const globalConfig = await readClaudeConfig().catch(() => ({}));
    ...
    try {
        ...
        for (const entry of sessionFiles) {
            ...
            try {
                ...
                if (session && typeof session.cwd === 'string' && session.cwd.trim()) {
                    activeDirs.push(session.cwd.trim());
                }
            } catch {
        ...
        for (const baseDir of activeDirs) {
            const candidates = [
            ...
            for (const candidate of candidates) {
                try {
                    ...
                    const config = normalized.trim() ? JSON.parse(normalized) : {};
                    return { config, path: candidate };
                } catch (error) {
                    if (error.code !== 'ENOENT') {
                        logger.warn(`[WebUI] Failed to read active Claude config ${candidate}: ${error.message}`);
                    }
                }
            }
        }
    } catch (error) {
        logger.warn(`[WebUI] Could not resolve active Claude config: ${error.message}`);
    }

    return { config: globalConfig, path: globalPath };
}

async function applyLiveClaudeModel(model, activeDirs = []) {
    if (!model || typeof model !== 'string') {
        return [];
    }

    const targets = Array.from(new Set((activeDirs || []).filter(Boolean))).map((cwd) => ({ cwd, sessionId: null }));
    if (targets.length === 0) {
        const sessionsDir = path.join(os.homedir(), '.claude', 'sessions');
    ...
    const uniqueTargets = targets.filter((target) => {
        const key = target.cwd + '::' + (target.sessionId || '');
    ...
    for (const target of uniqueTargets) {
        const result = await new Promise((resolve) => {
            ...
            const child = spawn(
                command,
                ['--continue', '--model', model, '-p', 'Respond with exactly: synced'],
                {
                    cwd: target.cwd,
                    env,
            ...
            const timer = setTimeout(() => {
                ...
                resolve({
                    ...target,
                    ok: false,
                    error: 'Timed out while applying live model'
                });
            }, 20000);
...
function validateConfigFields(input) {
    const updates = {};
    const {
        maxRetries, retryBaseMs, retryMaxMs, defaultCooldownMs, maxWaitBeforeErrorMs, maxAccounts,
        globalQuotaThreshold, accountSelection, rateLimitDedupWindowMs, maxConsecutiveFailures,
        extendedCooldownMs, maxCapacityRetries, switchAccountDelayMs, capacityBackoffTiersMs,
        ollamaBaseUrl, ollamaBaseUrls, ollamaCloudBaseUrl, ollamaApiKeys, openaiBaseUrl, openaiApiKeys, kimiBaseUrl, kimiApiKeys, ollamaFailoverCooldownMs
    } = input;
    ...
    if (Array.isArray(ollamaBaseUrls)) {
        const cleaned = ollamaBaseUrls
            .filter(v => typeof v === 'string')
            .map(v => v.trim())
            .filter(Boolean);
        if (cleaned.length <= 100) {
            updates.ollamaBaseUrls = [...new Set(cleaned)];
        }
    }
    if (typeof ollamaCloudBaseUrl === 'string') {
        updates.ollamaCloudBaseUrl = ollamaCloudBaseUrl.trim();
    }
    if (Array.isArray(ollamaApiKeys)) {
        const cleanedKeys = ollamaApiKeys
            .filter(v => typeof v === 'string')
            .map(v => v.trim())
            .filter(Boolean);
        if (cleanedKeys.length <= 100) {
            updates.ollamaApiKeys = cleanedKeys;
        }
    }
    if (typeof openaiBaseUrl === 'string') {
        updates.openaiBaseUrl = openaiBaseUrl.trim();
    }
    if (Array.isArray(openaiApiKeys)) {
        const cleanedKeys = openaiApiKeys
            .filter(v => typeof v === 'string')
            .map(v => v.trim())
            .filter(Boolean);
        if (cleanedKeys.length <= 100) {
            updates.openaiApiKeys = cleanedKeys;
        }
    }
    if (typeof kimiBaseUrl === 'string') {
        updates.kimiBaseUrl = kimiBaseUrl.trim();
    }
    if (Array.isArray(kimiApiKeys)) {
        const cleanedKeys = kimiApiKeys
            .filter(v => typeof v === 'string')
            .map(v => v.trim())
            .filter(Boolean);
        if (cleanedKeys.length <= 100) {
            updates.kimiApiKeys = cleanedKeys;
        }
    }
    if (typeof ollamaFailoverCooldownMs === 'number' && ollamaFailoverCooldownMs >= 1000 && ollamaFailoverCooldownMs <= 300000) {
        updates.ollamaFailoverCooldownMs = ollamaFailoverCooldownMs;
    }
...
export function mountWebUI(app, dirname, accountManager) {
    ...
    app.get('/api/config', (req, res) => {
        try {
            ...
            res.json({
                ...
                note: 'Edit ~/.config/modelhub-proxy/config.json or use env vars to change these values'
            });
        } catch (error) {
    ...
    app.get('/api/claude/config', async (req, res) => {
        try {
            const { config: claudeConfig, path: claudePath } = await getPreferredClaudeConfig();
            res.json({
                status: 'ok',
                config: claudeConfig,
                path: claudePath
    ...
    app.post('/api/claude/config', async (req, res) => {
        try {
            ...
            try {
                newConfig = await updateClaudeConfig(updates);
            } catch (error) {
                logger.warn(`[WebUI] Global Claude config update failed, continuing with active-session sync: ${error.message}`);
                const existingConfig = await readClaudeConfig().catch(() => ({}));
                newConfig = {
                    ...existingConfig,
                    ...updates,
                    env: {
                ...
                if (updates?.env?.ANTHROPIC_MODEL) {
                    newConfig.model = updates.env.ANTHROPIC_MODEL;
                }
            }

            // Mirror the active Claude session configs directly so Apply uses
            // the same files the running CLI session is watching.
            ...
            try {
                ...
                for (const entry of sessionFiles) {
                    ...
                    try {
                        ...
                        const session = raw.trim() ? JSON.parse(raw) : {};
                        if (session && typeof session.cwd === 'string' && session.cwd.trim()) {
                            activeDirs.add(session.cwd.trim());
                        }
                    } catch {
                        // Ignore malformed session files.
                    }
                    ...
                    for (const targetPath of targetPaths) {
                        try {
                            ...
                            try {
                                ...
                                const normalized = raw.replace(/^\uFEFF/, '');
                                existing = normalized.trim() ? JSON.parse(normalized) : {};
                            } catch (error) {
                            ...
                            logger.warn(`[WebUI] Skip syncing active Claude config ${targetPath}: ${error.message}`);
                        }
                    }
                }
            } catch (error) {
                logger.warn(`[WebUI] Could not mirror active Claude session configs: ${error.message}`);
            }

            let liveApply = [];
            if (updates?.env?.ANTHROPIC_MODEL) {
                try {
                    liveApply = await applyLiveClaudeModel(updates.env.ANTHROPIC_MODEL, Array.from(activeDirs || []));
                } catch (error) {
                    logger.warn(`[WebUI] Live Claude model apply failed: ${error.message}`);
                }
            }
    ...
    // ==========================================
    // Codex CLI Configuration API
    // ==========================================

    /**
     * GET /api/codex/config - Get Codex CLI proxy configuration
     */
    app.get('/api/codex/config', async (req, res) => {
        try {
            const codexState = await readCodexProxyConfig();
            res.json({
                status: 'ok',
                mode: codexState.mode,
                config: codexState.config,
                path: codexState.path || getCodexConfigPath()
            });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
    ...
    /**
     * POST /api/codex/config - Save Codex CLI proxy settings and enable proxy mode
     */
    app.post('/api/codex/config', async (req, res) => {
        try {
            const updates = req.body;
            if (!updates || typeof updates !== 'object') {
                return res.status(400).json({ status: 'error', error: 'Invalid Codex config updates' });
            }

            const result = await setCodexProxyConfig(updates);
            res.json({
                status: 'ok',
                mode: result.mode,
                config: result.config,
                path: result.path,
                message: 'Codex CLI proxy configuration updated'
            });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
    ...
    /**
     * GET /api/codex/mode - Get current Codex CLI mode
     */
    app.get('/api/codex/mode', async (req, res) => {
        try {
            const codexState = await readCodexProxyConfig();
            res.json({ status: 'ok', mode: codexState.mode });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
    });
    ...
    /**
     * POST /api/codex/mode - Switch Codex CLI between proxy and paid mode
     * Body: { mode: 'proxy' | 'paid', config?: { ... } }
     */
    app.post('/api/codex/mode', async (req, res) => {
        try {
            const { mode, config: incomingConfig } = req.body || {};
            if (!mode || !['proxy', 'paid'].includes(mode)) {
                return res.status(400).json({
                    status: 'error',
                    error: 'mode must be "proxy" or "paid"'
            ...
            if (mode === 'proxy') {
                const result = await setCodexProxyConfig(incomingConfig || {});
                return res.json({
                    status: 'ok',
                    mode: 'proxy',
                    config: result.config,
                    path: result.path,
                    message: 'Switched Codex CLI to Proxy mode. Restart Codex CLI to apply.'
                });
            }

            const result = await setCodexPaidMode();
            return res.json({
                status: 'ok',
                mode: 'paid',
                config: result.config,
                path: result.path,
                message: 'Switched Codex CLI to Paid mode. Restart Codex CLI to apply.'
            });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
    ...
    /**
     * GET /api/codex/presets - Get all saved Codex presets
     */
    app.get('/api/codex/presets', async (req, res) => {
        try {
    ...
    /**
     * POST /api/codex/presets - Save/update a Codex preset
     */
    app.post('/api/codex/presets', async (req, res) => {
        try {
            const { name, config: presetConfig } = req.body || {};
            if (!name || typeof name !== 'string' || !name.trim()) {
                return res.status(400).json({ status: 'error', error: 'Preset name is required' });
            ...
            const presets = await saveCodexPreset(name.trim(), presetConfig);
            res.json({ status: 'ok', presets, message: `Preset "${name}" saved` });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
    ...
    /**
     * DELETE /api/codex/presets/:name - Delete a Codex preset
     */
    app.delete('/api/codex/presets/:name', async (req, res) => {
        try {
            const { name } = req.params;
            if (!name) {
                return res.status(400).json({ status: 'error', error: 'Preset name is required' });
            }
            const presets = await deleteCodexPreset(name);
            res.json({ status: 'ok', presets, message: `Preset "${name}" deleted` });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
     ...
     */
    /**
     * GET /api/models/all - Return model IDs for UI selectors
     * Includes Ollama models so they appear in dropdowns even without cloud quota rows.
     */
    app.get('/api/models/all', async (req, res) => {
        try {
            const ids = new Set();

            // Include models from current account quota state
            const status = accountManager.getStatus();
            for (const acc of status.accounts || []) {
                const limits = acc.limits || {};
                for (const modelId of Object.keys(limits)) ids.add(modelId);
            }


            // Always include recommended baseline model IDs so users can select them
            // even before remote provider catalog calls succeed.
            ...
            // Include provider model catalogs
            for (const source of [
                { name: 'Ollama', fn: listOllamaModels },
                { name: 'OpenAI', fn: listOpenAIModels },
                { name: 'Kimi', fn: listKimiModels }
            ]) {
                try {
                    const payload = await source.fn();
                    for (const model of payload?.data || []) {
                        if (model?.id) ids.add(model.id);
                    }
                } catch (error) {
                    logger.warn(`[WebUI] Unable to include ${source.name} models in /api/models/all: ${error.message}`);
                }
            }

            res.json({ status: 'ok', models: Array.from(ids).sort() });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
    });
    ...
    /**
     * POST /api/ollama/test - Send a simple test prompt through Ollama routing
     */
