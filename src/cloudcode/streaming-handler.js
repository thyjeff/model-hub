    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (availableAccounts.length === 0) {
            ...
            if (accountManager.isAllRateLimited(model)) {
                ...
                // If wait timeout is enabled and wait time is too long, fail fast.
                // MAX_WAIT_BEFORE_ERROR_MS <= 0 means wait indefinitely until an account resets.
                if (MAX_WAIT_BEFORE_ERROR_MS > 0 && minWaitMs > MAX_WAIT_BEFORE_ERROR_MS) {
        ...
        try {
            ...
            while (endpointIndex < MODEL_HUB_ENDPOINT_FALLBACKS.length) {
                const endpoint = MODEL_HUB_ENDPOINT_FALLBACKS[endpointIndex];
                try {
